...
public class UserFileParser
{
	...
	boolean ret = true;
        ...
        try {
            ...
            Log.debug(this, "Caught exception: "+e.getMessage());
            document = null;
	    ret = false;
        }
	return ret;
	...
	if( (userName == null) || userName.equals("") ) {
	    Log.debug(this, "handleUserTag(): no name -attribute in <user> tag");
	} else {
	...
	if( chanAttrs != null ) {
		...
		Log.debug(this, "handleChannelTag(): no name " + 
			  "-attribute in <channel> tag");
	    } else {
		UserChannelInfo channelInfo = 
		    new UserChannelInfo(chanName,chanFlags);
		if( !user.addChannelInfo(channelInfo) ) {
		    Log.debug(this, "handleChannelTag(): channel " +
			      chanName + " already defined for user " +
			      user.getName());
		}				    
	    }
	}
