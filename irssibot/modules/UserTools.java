...
{
    /* statics */
    private static String moduleInfo = "User Tools $Revision: 1.2 $ for IrssiBot";
	...
	if( args[0].equals("hostmask") ) {
	  if ( args.length < 3 ) {
		...
		if( (invoker == user) || invoker.isChanAdmin(channel.getChannelName()) ||
		    ...
		    if ( user.hasMatchingHost(newHost) ) {
		      write("User already has host " + newHost);
		    } else {
		      // if valid hostmask, add to hosts list for user 
		      if( newHost.getNick() != null ) {
			user.addHost(newHost);
			write("Added new host " + newHost.toString() + 
			      " to user " + user.getName() + "." + notice);
	  ...
	  // must have user name as 2nd argument 
		...
		if( invoker.isChanAdmin(channel.getChannelName()) || invoker.isGlobalAdmin() ) {
		    ...
		    if ( caller.findUser(userName) != null ) {
		      write("User " + userName + " already exists.");
		      return;
		    }

		    User user = new User(userName, globalFlags);
		    caller.addUser(user);
