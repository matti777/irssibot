MySQL Connector/J
   ...
   Copyright (c) 1997-2008 MySQL AB, 2009 Sun Microsystems, Inc.
   All rights reserved. U.S. Government Rights - Commercial
   software. Government users are subject to the Sun
   Microsystems, Inc. standard license agreement and applicable
   provisions of the FAR and its supplements. Use is subject to
   license terms. Sun, Sun Microsystems, the Sun logo, Java,
   Solaris, StarOffice, MySQL Enterprise Monitor 2.0, MySQL
   logo(tm) and MySQL(tm) are trademarks or registered
   trademarks of Sun Microsystems, Inc. in the U.S. and other
   countries. UNIX is a registered trademark in the U.S. and
   other countries, exclusively licensed through X/Open Company,
   Ltd.

   Copyright (c) 1997-2008 MySQL AB, 2009 Sun Microsystems, Inc.
   Tous droits reserves. L'utilisation est soumise aux termes du
   contrat de licence.Sun, Sun Microsystems, le logo Sun, Java,
   Solaris, StarOffice, MySQL Enterprise Monitor 2.0, MySQL
   logo(tm) et MySQL(tm) sont des marques de fabrique ou des
   marques deposees de Sun Microsystems, Inc. aux Etats-Unis et
   dans d'autres pays. UNIX est une marque deposee aux
   Etats-Unis et dans d'autres pays et licenciee exlusivement
   ...
   Document generated on: 2009-07-14 (revision: 15667)
...
2.3.1. Upgrading from MySQL Connector/J 3.0 to 3.1
   ...
   enabled or disabled via configuration properties.
     ...
     * ResultSet.getString() --- Calling ResultSet.getString()
       ...
       represents it, instead of a String representation of the
       BLOB (http://dev.mysql.com/doc/refman/5.0/en/blob.html).
       BLOB (http://dev.mysql.com/doc/refman/5.0/en/blob.html)
       values have no character set, so they cannot be converted
...
shell> bzr branch lp:connectorj/5.1
    ...
    3. Change location to either the connectorj or 5.1
...
Chapter 3. Connector/J Examples
   ...
   examples.
     ...
     * Example 5.1.1, "Connector/J: Obtaining a connection from
       the DriverManager"
     ...
     * Example 5.1.3, "Connector/J: Using
       Connection.prepareCall()"

     * Example 5.1.3, "Connector/J: Registering output
       parameters"

     * Example 5.1.3, "Connector/J: Setting CallableStatement
       input parameters"

     * Example 5.1.3, "Connector/J: Retrieving results and
       output parameter values"

     * Example 5.1.4, "Connector/J: Retrieving AUTO_INCREMENT
     ...
     * Example 5.2.1.1, "Connector/J: Using a connection pool
       with a J2EE application server"

     * Example 5.3, "Connector/J: Example of transaction with
       retry logic"
...
Note
   ...
   localSocketAddress Host name or IP address given to
   explicitly configure the interface that the driver will bind
   ...
   consistency when applications do not handle SQLExceptions
   ...
   selfDestructOnPingMaxOperations =If set to a nonzero value,
   ...
   selfDestructOnPingSecondsLifetime If set to a nonzero value,
   the driver will report close the connection and report
   failure when Connection.ping() or Connection.isValid(int) is
   ...
   used for XAResource.isSameRM() when the driver cannot
   determine this value based on host names used in the URL
   5.0.1
   ...
   enabled, what is the largest SQL the driver will cache the
   ...
   rewrite option, and that if you do not specify stream lengths
   when using PreparedStatement.set*Stream(), the driver will
   not be able to determine the optimum number of parameters per
   ...
   useReadAheadInput Use newer, optimized nonblocking, buffered
   ...
   true, and this property is set to a nonzero value, the driver
   will use this threshold (in nanosecond units) to determine if
   a query was slow. 0 5.0.7
   ...
   cannot determine the character set mapping, or you are trying
   ...
   (4-byte encodings), which MySQL server cannot handle
   natively. false 5.1.3
   ...
   ignoreNonTxTables Ignore nontransactional table warning for
   ...
   user cannot access procedure bodies through "SHOW CREATE
   ...
   noDatetimeStringSync Do not ensure that
   ResultSet.getDatetimeType().toString().equals(ResultSet.getSt
   ring()) false 3.1.7
   noTimezoneConversionForTimeType Do not convert TIME values
   ...
   useOnlyServerErrorMessages Do not prepend 'standard' SQLState
   ...
   useUnbufferedInput Do not use BufferedInputStream for reading
...
4.4. Using Character Sets and Unicode
   ...
   Set and Collation
   (http://dev.mysql.com/doc/refman/5.0/en/charset-syntax.html#c
   harset-server).
...
VendorError: 0
     ...
     * 5.3.3: I'm trying to use MySQL Connector/J in an applet
...
VendorError: 0
     ...
     * 5.3.5: I'm trying to use JDBC-2.0 updatable result sets,
     ...
     * 5.3.7: I am trying to connect to my MySQL server within
...
va:411)
     ...
     * 5.3.9: When using gcj an java.io.CharConversionException
       is raised when working with certain character sequences.

     * 5.3.10: Updating a table that contains a primary key that
       is either FLOAT
...
Note
   ...
   (http://dev.mysql.com/doc/refman/5.0/en/mysql-description.htm
   l#option_mysql_host) flag, and use something other than
   localhost for the host. The mysql command-line client will
...
Note
   ...
   system. For more information, see Can't connect to [local]
   MySQL server
   (http://dev.mysql.com/doc/refman/5.0/en/common-errors.html#ca
...
A.1.1. Changes in MySQL Connector/J 5.1.8 (Not yet released)

   Bugs fixed:

     * The reported milliseconds since the last server packets
...
server was 1 milliseconds ago.
     ...
     * Calling Connection.serverPrepareStatement() variants that
     ...
     * The result set returned by getIndexInfo() did not have
     ...
     * The DEFERRABILITY column in database metadata result sets
     ...
     * The result set returned by getVersionColumns() did not
     ...
     * The result set returned by getBestRowIdentifier() did not
       have the format defined in the JDBC API specifications.
       The third column, DATA_TYPE, of the result set should be
       of type INTEGER. Connector/J however returns SMALLINT.
...
...
     ...
     * Statement.getGeneratedKeys() retained result set
...
DatabaseMetaDataUsingInfoSchema.java:603)
     ...
     * LoadBalancingConnectionProxy.doPing() did not have
       blacklist awareness.
     ...
     * When connecting with traceProtocol=true, no trace data
...
tionProxy.java:390)
     ...
     * If there was an apostrophe in a comment in a statement
...
String sql = "-- Customer's zip code will be fixed\n" +
  ...
  "where not regexp '^[0-9]{5}([[.-.]])?([0-9]{4})?$'";
       ...
       did not appear to be in quotes. With the parser not
       detecting that the regular expression was in quotes, the
       curly braces were recognized as escape sequences and were
       removed from the regular expression, breaking it. The
       server thus received SQL such as:
-- Customer's zip code will be fixed
update address set zip_code = '99999'
where not regexp '^[0-9]([[.-.]])?([0-9])?$'
       (Bug#41566: http://bugs.mysql.com/41566)

     * MySQL Connector/J 5.1.7 was slower than previous versions
...
at com.zimbra.soap.SoapEngine.dispatch(SoapEngine.java:285)
     ...
     * When DatabaseMetaData.getProcedureColumns() was called,
     ...
     * PreparedStatement.addBatch() did not check for all
     ...
     * Error message strings contained variable values that were
...
SELECT TRIM(rowid) FROM tbl
       ...
       type. (Bug#38387: http://bugs.mysql.com/38387)
...
A.1.2. Changes in MySQL Connector/J 5.1.7 (21 October 2008)
   ...
   Bugs fixed:
     ...
     * Statement.getGeneratedKeys() returned two keys when using
       ON DUPLICATE KEY UPDATE and the row was updated, not
       inserted. (Bug#42309: http://bugs.mysql.com/42309)
     ...
     * Connector/J failed to parse TIMESTAMP
       ...
       (Bug#39911: http://bugs.mysql.com/39911)
...
nsToHostsMap.get(this.currentConn))).intValue();
     ...
     * When configuring the Java Replication Driver the last
...
g to data
     ...
     * When getGeneratedKeys() was called on a statement that
...
SELECT column AS aliasName from table
     ...
     * ResultSet.getTimestamp() returns incorrect values for
       ...
       by default). (Bug#34913: http://bugs.mysql.com/34913)
     ...
     * During a Daylight Savings Time (DST) switchover, there
       ...
       (that is, the old date/time handling code will be
       deprecated) (Bug#32577: http://bugs.mysql.com/32577,
...
A.1.6. Changes in MySQL Connector/J 5.1.3 (10 September 2007)
   ...
   Functionality added or changed:
     ...
     * The data (and how it is stored) for ResultSet rows are
...
useServerPrepStmts=true
   ...
   Bugs fixed:
     ...
     * Using setFetchSize() breaks prepared SHOW
       ...
       other commands. (Bug#24360: http://bugs.mysql.com/24360)
...
A.3.3. Changes in MySQL Connector/J 3.1.13 (26 May 2006)
   ...
   Bugs fixed:
     ...
     * DBMD.getColumns() returns wrong type for BIT
       (http://dev.mysql.com/doc/refman/5.0/en/numeric-types.htm
       l). (Bug#15854: http://bugs.mysql.com/15854)
...
A.3.4. Changes in MySQL Connector/J 3.1.12 (30 November 2005)
   ...
   Bugs fixed:
     ...
     * Java type conversion may be incorrect for MEDIUMINT
       (http://dev.mysql.com/doc/refman/5.0/en/numeric-types.htm
       l). (Bug#14562: http://bugs.mysql.com/14562)
...
A.3.8. Changes in MySQL Connector/J 3.1.8 (14 April 2005)
   ...
   Bugs fixed:
     ...
     * A continuation of Bug#8868: http://bugs.mysql.com/8868,
       ...
       nonstring types when resolved by temporary tables
     ...
     * PreparedStatement.addBatch() doesn't work with
       ...
       (http://dev.mysql.com/doc/refman/5.0/en/binary-varbinary.
       html) data. (Bug#9040: http://bugs.mysql.com/9040)
     ...
     * DATE_FORMAT()
       ...
       from getObject(). (Bug#8868: http://bugs.mysql.com/8868)
...
A.3.12. Changes in MySQL Connector/J 3.1.4 (04 September 2004)
   ...
   Bugs fixed:
     ...
     * ServerPreparedStatements dealing with return of DECIMAL
       (http://dev.mysql.com/doc/refman/5.0/en/numeric-types.htm
       l) type don't work.
       (Bug#5012: http://bugs.mysql.com/5012)
...
A.4.2. Changes in MySQL Connector/J 3.0.16 (15 November 2004)
   ...
   Bugs fixed:
     ...
     * Made TINYINT(1) -> BIT
       ...
       box). (Bug#5664: http://bugs.mysql.com/5664)
     ...
     * Fixed DatabaseMetaData.getTypes() returning incorrect
       ...
       (http://dev.mysql.com/doc/refman/5.0/en/numeric-types.htm
       l) type. (Bug#5664: http://bugs.mysql.com/5664)
...
A.4.3. Changes in MySQL Connector/J 3.0.15 (04 September 2004)
   ...
   Bugs fixed:
     ...
     * RSMD.getPrecision() returning 0 for nonnumeric types
...
A.4.6. Changes in MySQL Connector/J 3.0.12 (18 May 2004)
   ...
   Bugs fixed:
     ...
     * Only set character_set_results
       ...
       4.1.0. (Bug#2670: http://bugs.mysql.com/2670)
...
A.4.7. Changes in MySQL Connector/J 3.0.11 (19 February 2004)
   ...
   Bugs fixed:
     ...
     * Detect on/off or 1, 2, 3 form of lower_case_table_names
       ...
       server. (Bug#2578: http://bugs.mysql.com/2578)
...
A.4.8. Changes in MySQL Connector/J 3.0.10 (13 January 2004)
   ...
   Bugs fixed:
     ...
     * ResultSetMetaData.isCaseSensitive() returned wrong value
       ...
       (http://dev.mysql.com/doc/refman/5.0/en/char.html)/VARCHA
       R (http://dev.mysql.com/doc/refman/5.0/en/char.html)
       columns. (Bug#1913: http://bugs.mysql.com/1913)
...
A.4.15. Changes in MySQL Connector/J 3.0.3 (17 December 2002)
   ...
   Bugs fixed:
     ...
     * Use nonaliased table/column names and database names to
